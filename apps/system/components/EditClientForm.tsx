...
export default function EditClientForm({
  ...
  const [formData, setFormData] = useState({
    ...
    unitId: client.unitId ? String(client.unitId) : "",
  });
    ...
    const fetchUnits = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // FILTER: Show 'available' units OR the unit currently assigned to this client
          const allowedUnits = data.filter(
            (u: Unit) =>
              u.unitStatus === "available" ||
              String(u.id) === String(client.unitId),
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(
          data.error || data.message || "Failed to update client",
        );
      }
